<?php
	...
	class demodizzyInstallScenario extends siteInstallScenario implements iSiteInstallScenario {
		...
		public function runViaDomain($id) {
			...
			$this->addAppointmentData($id);
			$this->setUsersSecuritySettings();
			$this->saveSvPhone();
			$this->createApiShipDelivery($id);
			$this->setCatalogObjectsWeight($id);
			$this->setWeightField();
			$this->setTaxRateField($id);
		}
		 ...
		 */
		private function setTaxRateField($domainId) {
			...
			foreach ($childrentTypeIdList as $childrenTypeId) {
				...
				if ($childrenType->getDomainId() != $domainId) {
					continue;
				}
		 ...
		 */
		private function setCatalogObjectsWeight($domainId) {
			$query = new selector('pages');
			$query->types('object-type')->name('catalog', 'object');
			$query->where('domain')->equals($domainId);
		 ...
		 * @throws coreException
		 */
		private function createApiShipDelivery($domainId) {
			...
			$apiShipDelivery->setValue('domain_id_list', [$domainId]);
		 ...
		 * @throws coreException
		 */
		private function addAppointmentData($domainId) {
			$appointment = cmsController::getInstance()->getModule('appointment');

			if ($appointment instanceof appointment) {
				$this->addAppointmentPage($domainId);
		 ...
		 * @throws coreException
		 */
		private function addAppointmentPage($domainId) {
