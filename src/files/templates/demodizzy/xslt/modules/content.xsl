...
<xsl:stylesheet	version="1.0"
	...
	<xsl:template match="result[@module = 'content' and @method = 'content']">
		<div umi:element-id="{/result/@pageId}" umi:field-name="content" umi:empty="&empty-page-content;">
	...
	<xsl:template match="udata[@method = 'menu']//item">
		<li umi:region="row">
			<a href="{@link}" umi:element-id="{@id}" umi:field-name="name" umi:delete="delete" umi:empty="&empty-page-name;">
				<xsl:value-of select="." />
			</a>
		</li>
	</xsl:template>
